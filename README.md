...
> **版本**: v1.3.0 (NIDS + HIDS 混合检测 + 多级导航 + 赛博朋克UI)  
> **状态**: 生产就绪 (Production Ready) | Major UI/UX Update  
> **文档日期**: 2025-12-14  
> **核心架构**: Spring Boot 微服务 + React 可视化 + TransecGAN 深度学习 + Hyperledger Fabric 区块链 + GenAI 决策支持

---

## v1.3.0 更新日志 (What's New)

### UI/UX 全面升级
| 模块 | 更新内容 |
|------|----------|
| **NIDS 网络入侵检测** | 重命名：实时威胁预警 → NIDS 网络入侵检测<br>多级导航：风险等级 → 攻击类型概览 → 攻击详情<br>面包屑导航支持快速返回任意层级 |
| **HIDS 主机监控** | 新增：用户登录监控（成功/失败统计）<br>新增：安全基线检查（防火墙/杀毒/更新状态）<br>新增：主机安全告警（CPU/内存/磁盘异常检测）<br>新增：NIDS网络威胁概览卡片（实时同步） |
| **网络连接监控** | 增强：统计卡片（峰值/谷值/平均值）<br>新增：活跃端口监控<br>图表升级：折线图 → 面积图 |
| **进程监控** | 重构：空洞入口 → 实用进程面板<br>新增：运行进程数、可疑进程数统计<br>新增：CPU占用Top 3进程列表 |

### 功能增强
- **威胁历史扩容**: pageSize 100 → 1000，支持查看更多历史数据
- **实时刷新**: NIDS威胁概览每10秒自动更新
- **交互优化**: 所有卡片添加hover效果、点击跳转、状态感知

### Bug修复
- 修复视图模式切换时的TypeScript类型错误
- 修复未使用变量的lint警告
...
### 1.1 核心价值
*   **NIDS + HIDS 混合检测**: 网络层与主机层双重防护，实现全方位威胁感知
*   **零日威胁免疫**: 摒弃传统基于特征库的检测模式，采用 **TransecGAN** (Transformer-Encoder GAN) 模型，有效识别未知 Zero-day 攻击
*   **智能决策辅助**: 集成生成式 AI 报告引擎，自动聚合分散的威胁数据，生成符合 CISO 决策标准的安全日报/周报
*   **主动防御 (Active Response)**: HIDS 探针不仅仅是监控器，更是执行器。它能联动系统防火墙实现毫秒级自动封禁
*   **司法级存证**: 引入 **Hyperledger Fabric** 联盟链，将关键威胁告警实时上链，确保数据不可篡改
*   **赛博朋克可视化**: 沉浸式霓虹风格 UI，多级导航交互，提供专业级安全态势感知体验
...
### 7.2 NIDS 网络入侵检测 (v1.3.0 新增多级导航)
*   **入口**: 侧边栏"NIDS 网络入侵检测"。
*   **功能**:
    *   **风险概览**: 高危/中危/低危威胁卡片，点击进入对应分类
    *   **攻击类型概览**: 详细统计每种攻击类型的数量、特征分析、24小时趋势
    *   **攻击详情列表**: 展开查看源IP、目标IP、时间戳等完整信息
    *   **面包屑导航**: 支持快速跳转回任意层级
    *   **一键封禁**: 对恶意IP执行防火墙阻断

### 7.3 HIDS 主机监控 (v1.3.0 全面增强)
*   **入口**: 侧边栏"HIDS 主机监控"。
*   **模块**:
    *   **CPU/内存监控**: 实时面积图展示负载变化
    *   **进程监控**: Top 3 CPU占用进程、运行进程数、可疑进程检测
    *   **网络连接**: 峰值/谷值/平均值统计、活跃端口监控
    *   **用户登录监控**: 成功/失败登录统计
    *   **安全基线检查**: 防火墙/杀毒软件/系统更新状态
    *   **主机安全告警**: 基于CPU/内存/磁盘的异常检测
    *   **NIDS威胁概览**: 实时显示网络层威胁统计（点击跳转详情）

### 7.4 防火墙管理 (Firewall Panel)
