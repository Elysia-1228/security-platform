...
CREATE TABLE `host_status_monitor`  (
  ...
  `cpu_freq` double NULL COMMENT 'CPU主频(GHz)',
  `memory_info` varchar(255) NULL COMMENT '内存信息',
  `memory_total_gb` double NULL COMMENT '内存总量(GB)',
  `memory_used_gb` double NULL COMMENT '内存已用(GB)',
  `disk_total_gb` bigint NULL COMMENT '磁盘总量(GB)',
  `disk_used_gb` bigint NULL COMMENT '磁盘已用(GB)',
  `disk_free_gb` bigint NULL COMMENT '磁盘可用(GB)',
  `disk_partitions` text NULL COMMENT '磁盘分区详情(JSON)',
  PRIMARY KEY (`id`) USING BTREE,
